                                ...
                                <div class="col-md-12">
                                    ...
                                    <ul class="timeline">
                                        <li ng-repeat="track in timeline " ng-class-even="'timeline-inverted'">
                                            <span ng-class-even="'timeline-inverted'"> @{{track.updated_at_date}}</span>

                                            <div class="timeline-badge" ng-class="track.id">
                                                <h4 style="margin-top: 15px !important;">@{{track.updated_at_time}}</h4>
                                            </div>
                                            <div class="timeline-panel">
                                                <div class="timeline-heading">
                                                    <b> <h4 class="timeline-title">@{{track.title}}</h4></b>
                                                </div>
                                                <div class="timeline-body">
                                                    <label ng-if="track.status">Status: </label> <span style="text-transform:lowercase;!important">@{{track.status}}</span>
                                                    <label ng-if="track.created_by">Created By:</label> <span>@{{track.created_by}}</span>
                                                    <label ng-if="track.assigned_to">Assigned To:</label> <span>@{{track.assigned_to}}</span>
                                                    <label ng-if="track.assigned_by">Assigned By:</label> <span>@{{track.assigned_by}}</span>
                                                    <label ng-if="track.completed_by">Completed By:</label> <span>@{{track.completed_by}}</span>
                                                    <br>
                                                    <label ng-if="track.reason">Reason:</label> <span>@{{track.reason}}</span>
                                                    <br>
                                                    <label ng-if="track.description">Comments:</label> <span>@{{track.description}}</span>
                                                </div>
                                            </div>
                                        </li>
                                    </ul>
